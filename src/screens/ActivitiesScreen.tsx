...
import { Appbar, Avatar, Button, Menu, Searchbar, Switch, Text, TouchableRipple } from "react-native-paper";
...
const ActivitiesScreen = ({ route, navigation }: Props) => {
  ...
  const getMembers = async () => {
    ...
    const { res, data } = await ApiRequest<ActivityResponse>(
      `/activities/?page_size=30&${status ? "status=" + status : ""}${
  ...
  }, [user?.id, searchQuery, page, ordering, status]);
    ...
    navigation.setOptions({
      header: ({ navigation }) => (
        <Appbar.Header style={{ backgroundColor: theme.colors.primary }}>
          ...
          <Appbar.Action
            onPress={() => {
              setStatus((stat) => (stat !== "archive" ? "archive" : "future"));
            }}
            icon={status !== "archive" ? "archive" : "archive-outline"}
          />
        </Appbar.Header>
      ),
    });
      ...
      >
        <View style={styles.itemContainer}>
          ...
          <Switch
            ...
            disabled={_disabled(item) || new Date(item.registration_close).getTime() < new Date().getTime()}
          />
        </View>
      </TouchableRipple>
    );
